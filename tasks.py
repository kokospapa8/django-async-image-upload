...
def coverage(c):
    ...
    """
    c.run("coverage run --source django-async-image-upload runtests.py tests")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/django-async-image-upload.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ django_async_image_upload")
...
def lint(c):
    ...
    """
    c.run("flake8 django-async-image-upload tests")
...
def release(c, bumpsize=''):
    ...
    import django_async_image_upload
    c.run("python setup.py sdist bdist_wheel")
    c.run("twine upload dist/*")

    c.run('git tag -a {version} -m "New version: {version}"'.format(version=django_async_image_upload.__version__))
    c.run("git push --tags")
